...
disk_area_per_rotor = 2.8    # m² per rotor (based on DL ≈ 700 N/m²)
N_rotors = 4
...
# Diversion allowance (10% of mission range)
R_div = 0.1 * range_m
...
def m_motor(P_cb):
    """Motor mass (kg) for all motors combined"""
...
def P_descent(P_hv, v_hv, Vv=-V_v):
    """Full descent power model (Eq. 6 from Ugwueze et al., 2023)"""
    ratio = Vv / (2.0 * v_hv)
    if ratio <= -2.0:
        return P_hv * (ratio + np.sqrt(ratio**2 - 1.0))
    else:
        return P_hv  # For mild descent, approx = hover power
...
for i in range(1, iter_max + 1):
    ...
    t_cruise = (range_m + R_div) / V_cr # cruise + diversion
    t_descent = climb_ht / V_v          # vertical descent
    t_hover_ld = hover_ht / v_hv        # landing hover

    # ------------------------------------------
    # Total mission energy (J)
    # ------------------------------------------
    E_total = (P_hv * t_hover_to
              + P_cb * t_climb
              + P_cr * t_cruise
              + P_ds * t_descent
              + P_hv * t_hover_ld)
...
plt.axis('equal')
plt.savefig('mass_breakdown.png')
plt.show()
